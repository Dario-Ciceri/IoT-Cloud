...
const String clientServerIp = 'localhost'; //interno alla rete docker
const int clientServerPort = 9002; //interno alla rete docker
...
class AwsS3Client {
  ...
  AwsS3Client(
    ...
    print('AWS S3 Client Initialized with:');
    print('Host: $_host');
    print('Region: $_region');
    print('Bucket ID: $_bucketId');

    // Verifichiamo se l'host è stato fornito esplicitamente o è stato generato
    if (host != null) {
      print('Host fornito esplicitamente: $host');
    } else {
      print('Host generato automaticamente: $_host');
    }
  }
  ...
  SignedRequestParams buildSignedParams({
    ...
    final uri = Uri.http(_host, unencodedPath, queryParams);
    print('URI: $uri');

    final payload = SigV4.hashCanonicalRequest('');
    print('Payload (hash canonical request): $payload');

    final datetime = SigV4.generateDatetime();
    print('Datetime (AWS format): $datetime');
    ...
    print('Canonical Query: $canonicalQuery');
...
$payload''';
    ...
    // Rimuovi x-amz-security-token se non hai credenziali temporanee
    final authorization = [
      'AWS4-HMAC-SHA256 Credential=$_accessKey/$credentialScope',
      'SignedHeaders=host;x-amz-content-sha256;x-amz-date', // Rimuovi x-amz-security-token
      'Signature=$signature',
    ].join(',');
...
ListBucketResult? _parseListObjectResponse(String responseXml) {
  ...
  try {
    ...
    return ListBucketResultParker.fromJsonMap(
      fixedJson as Map<String, dynamic>,
    ).result;
  }
}
